const taskReducer = (
    ...
    switch (action.type) {
      ...
      case "UPDATE_TASK": {
        let newState = { ...state };
        let goalIndex = newState.Tasks.findIndex(g => {
          return g.id == action.payload.id;
        });
        newState.Tasks[goalIndex] = action.payload;
  
        goalIndex = newState.FilteredTasks.findIndex(g => {
          return g.id == action.payload.id;
        });
        newState.FilteredTasks[goalIndex] = action.payload;
        state = newState;
        break;
      }
      ...
      case "FILTER_TASKS_BY_STATUS": {
        let newState = { ...state };
        newState.FilteredTasks = getFilteredTasks(newState.Tasks,newState.CurrentFilterString,action.payload)
  ...
  function getFilteredTasks(goals, filterString,currentStatus) {
